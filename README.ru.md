...
На Wayland утилиты `xrandr` и `xdpyinfo` обычно недоступны; используется только screeninfo. Если количество подписей в CSV не совпадает с количеством изображений, скрипт выведет предупреждение и подставит заглушку для отсутствующих подписей.
